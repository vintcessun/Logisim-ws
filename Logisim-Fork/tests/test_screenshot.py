...
async def test_screenshot():
    ...
    async with websockets.connect(URI) as ws:
        ...
        await send_json(ws, "switch_circuit", name="8位无符号数一位乘法器")

        print(await send_json(ws, "get_io"))
        ...
        if isinstance(binary_data, bytes):
            ...
            if width < 1920 and height < 1080:
                ...
                print(
                    "⚠️ Warning: Image dimensions are 1920x1080, check if parameters were really ignored."
                )
        else:
